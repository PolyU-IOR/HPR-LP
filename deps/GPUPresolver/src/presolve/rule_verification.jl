...
function _missing_downstream_solver_for_rule_verification()
    error("Rule verification with recovered solutions requires an external downstream solver; GPUPresolver currently provides presolve/postsolve only.")
end
...
function build_isolated_presolve_params(
    ...
    pparams.enable_fme_projection = false
    pparams.enable_structural_l1_substitution = false
    ...
    if enabled
        ...
        elseif rule == :singleton_cols_dual_infer
            pparams.enable_singleton_cols_dual_infer = true
            pparams.col_rule_order = [:singleton_cols_dual_infer]
        elseif rule == :singleton_cols_eq
        ...
        elseif rule == :fme_projection
            pparams.enable_fme_projection = true
            pparams.col_rule_order = [:fme_projection]
        elseif rule == :structural_l1_substitution
            pparams.enable_structural_l1_substitution = true
            pparams.col_rule_order = [:structural_l1_substitution]
...
function build_isolated_singleton_cols_params(;
    max_iters::Int=20,
    verbose::Bool=false,
    debug_checks::Bool=false,
)
    ...
    pparams.enable_singleton_cols_dual_infer = true
    pparams.col_rule_order = [:singleton_cols_dual_infer, :singleton_cols_eq]
    return pparams
end
...
function diagnose_singleton_cols(
    lp::LP_info_gpu,
    pparams::PresolveParams=build_isolated_singleton_cols_params();
...
function diagnose_singleton_cols(
    ...
    model = build_from_mps(String(file), verbose)
    lp_gpu = setup_gpu_model(model; device_number=device_number, verbose=verbose)
    pparams = build_isolated_presolve_params(
        :singleton_cols_eq;
        max_iters=max_iters,
        verbose=verbose,
...
function validate_isolated_rule_postsolve(
    ...
    _ = (time_limit, presolve_time_limit, max_iter, stoptol, device_number, verbose, presolve_max_iters, debug_checks)
    rule in CURRENT_GPU_RULES || error("Unsupported presolve rule: $rule")
    isfile(file) || error("MPS file not found: $file")
    return _missing_downstream_solver_for_rule_verification()
end
...
function diagnose_isolated_bound_change_postsolve(
    ...
    _ = (time_limit, presolve_time_limit, max_iter, stoptol, device_number, verbose, presolve_max_iters, debug_checks, tol)
