...
function _kernel_count_mapped_rows_kept_cols!(
    row_nnz_new,
    rowPtr_org,
    colVal_org,
    row_red2org,
    ...
    r = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if r <= m1
        @inbounds row_org = row_red2org[r]
        @inbounds src_first = rowPtr_org[row_org]
        @inbounds src_last = rowPtr_org[row_org + 1] - 1
        cnt = Int32(0)
        if src_first <= src_last
            for p in src_first:src_last
                @inbounds col_org = colVal_org[p]
...
function _kernel_copy_mapped_rows_kept_cols_csr!(
    colVal_new,
    nzVal_new,
    rowPtr_new,
    rowPtr_org,
...
"""
Compact CSR by selected rows and columns in one pass.

This is equivalent to
`compact_csr_by_cols(compact_csr_by_rows(A_csr, row_red2org), col_red2org)`,
but avoids materializing the row-only intermediate matrix.
"""
function compact_csr_by_rows_and_cols(
    A_csr::CuSparseMatrixCSR{T,Int32},
    row_red2org::CuVector{Int32},
    col_red2org::CuVector{Int32},
) where {T}
    ...
    if n1 == 0
        rowPtr_new = CUDA.fill(Int32(1), m1 + 1)
        colVal_new = CuVector{Int32}(undef, 0)
        nzVal_new = CuVector{T}(undef, 0)
        return CuSparseMatrixCSR(rowPtr_new, colVal_new, nzVal_new, (m1, 0))
    end

    col_org2red = build_org2red_from_red2org(col_red2org, n0)
    row_nnz_new = CUDA.zeros(Int32, m1)
    blocks_rows = cld(m1, GPU_PRESOLVE_THREADS)
    @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks_rows _kernel_count_mapped_rows_kept_cols!(
        row_nnz_new,
        A_csr.rowPtr,
        A_csr.colVal,
        row_red2org,
    ...
    nnz_new = Int(_copy_scalar_to_host(prefix, m1))
    colVal_new = CuVector{Int32}(undef, nnz_new)
    nzVal_new = CuVector{T}(undef, nnz_new)

    if nnz_new > 0
        @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks_rows _kernel_copy_mapped_rows_kept_cols_csr!(
            colVal_new,
            nzVal_new,
            rowPtr_new,
            A_csr.rowPtr,
    ...
    return CuSparseMatrixCSR(rowPtr_new, colVal_new, nzVal_new, (m1, n1))
end
