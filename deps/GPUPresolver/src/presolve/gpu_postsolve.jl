...
function _kernel_structural_restore_splits!(
    x_org,
    t_cols,
    e_cols,
    rhos,
    ...
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if i <= k
        @inbounds t_idx = Int(t_cols[i])
        @inbounds e_idx = Int(e_cols[i])
        @inbounds rho = rhos[i]
        @inbounds t_new = x_org[t_idx]
        @inbounds e_new = x_org[e_idx]
        @inbounds x_org[t_idx] = t_new + e_new
        @inbounds x_org[e_idx] = (t_new - e_new) / rho
    end
    return
end

function _kernel_structural_restore_outer_pairs!(
    x_org,
    bound_cols,
    free_cols,
    k,
    ...
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if i <= k
        @inbounds x_org[Int(free_cols[i])] = x_org[Int(bound_cols[i])]
    end
    return
end

function _kernel_structural_restore_linked_slacks!(
    x_org,
    slack_cols,
    t_cols,
    factors,
    ...
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if i <= k
        @inbounds x_org[Int(slack_cols[i])] = factors[i] * x_org[Int(t_cols[i])]
    end
    return
end

function _kernel_structural_restore_max_slack!(
    x_org,
    slack_col,
    t_cols,
    factors,
    ...
    j = tid
    while j <= count
        @inbounds candidate = factors[j] * x_org[Int(t_cols[j])]
        partial = max(partial, candidate)
        j += lane_count
    end
    ...
    if tid == 1
        @inbounds x_org[Int(slack_col)] = scratch[1]
    end
    return
end

function postsolve_restore_structural_primal_gpu!(
    x_org::CuVector{Float64},
    rec::PresolveRecord_gpu,
    original_model_gpu::Union{Nothing,LP_info_gpu},
)
    isempty(rec.structural_primal_recoveries) && return x_org

    original_l = isnothing(original_model_gpu) ? nothing : original_model_gpu.l

    for step in Iterators.reverse(rec.structural_primal_recoveries)
        if !isempty(step.splits)
            t_cols = CuVector(Int32[split.t_col for split in step.splits])
            e_cols = CuVector(Int32[split.e_col for split in step.splits])
            rhos = CuVector(Float64[split.rho for split in step.splits])
            k = length(step.splits)
            blocks = cld(k, GPU_PRESOLVE_THREADS)
            @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks _kernel_structural_restore_splits!(
                x_org,
                t_cols,
                e_cols,
                rhos,
        ...
        if !isempty(step.outer_pairs)
            bound_cols = CuVector(Int32[pair.bound_col for pair in step.outer_pairs])
            free_cols = CuVector(Int32[pair.free_col for pair in step.outer_pairs])
            k = length(step.outer_pairs)
            blocks = cld(k, GPU_PRESOLVE_THREADS)
            @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks _kernel_structural_restore_outer_pairs!(
                x_org,
                bound_cols,
                free_cols,
                Int32(k),
        ...
        if !isempty(step.linked_slacks)
            slack_cols = CuVector(Int32[slack.slack_col for slack in step.linked_slacks])
            t_cols = CuVector(Int32[slack.t_col for slack in step.linked_slacks])
            factors = CuVector(Float64[slack.factor for slack in step.linked_slacks])
            k = length(step.linked_slacks)
            blocks = cld(k, GPU_PRESOLVE_THREADS)
            @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks _kernel_structural_restore_linked_slacks!(
                x_org,
                slack_cols,
                t_cols,
                factors,
        ...
        for slack in step.max_slacks
            lower_bound = 0.0
            if !isnothing(original_l)
                lb = _copy_scalar_to_host(original_l, Int(slack.slack_col))
                lower_bound = isfinite(lb) ? lb : 0.0
            end
            t_cols = CuVector(slack.t_cols)
            factors = CuVector(slack.factors)
            @cuda threads=GPU_PRESOLVE_THREADS blocks=1 _kernel_structural_restore_max_slack!(
                x_org,
                slack.slack_col,
                t_cols,
                factors,
                Int32(length(slack.t_cols)),
                lower_bound,
            )
        end
    end
...
function _kernel_mark_exact_replay_columns_from_tape_gpu!(
    ...
    if k <= record_count
        ...
        elseif reduction_type == Int32(DOUBLETON_EQ)
            elim_col = Int(@inbounds tape_idx[idx0])
            keep_col = Int(@inbounds tape_idx[idx0 + 1])
            if 1 <= elim_col <= n
                @inbounds protected_cols[elim_col] = UInt8(1)
            end
            if 1 <= keep_col <= n
                @inbounds protected_cols[keep_col] = UInt8(1)
            end
...
@inline function _doubleton_eq_recover_z_split(
    x_keep::Float64,
    z_keep_hat::Float64,
    alpha::Float64,
    old_keep_l::Float64,
    ...
    abs(alpha) > tol || return (z_keep_hat, 0.0)
    at_lower = isfinite(new_keep_l) && x_keep <= new_keep_l + tol
    at_upper = isfinite(new_keep_u) && x_keep >= new_keep_u - tol

    if !(at_lower || at_upper)
        return (0.0, 0.0)
    elseif at_lower
        source = lower_source
        if source == DOUBLETON_EQ_BOUND_UNKNOWN
            source = (
                isfinite(old_keep_l) && abs(new_keep_l - old_keep_l) <= tol
            ) ? DOUBLETON_EQ_BOUND_INHERITED : DOUBLETON_EQ_BOUND_INDUCED
        end
        return source == DOUBLETON_EQ_BOUND_INDUCED ?
               (0.0, z_keep_hat / alpha) :
               (z_keep_hat, 0.0)
    else
        source = upper_source
        if source == DOUBLETON_EQ_BOUND_UNKNOWN
            source = (
                isfinite(old_keep_u) && abs(new_keep_u - old_keep_u) <= tol
            ) ? DOUBLETON_EQ_BOUND_INHERITED : DOUBLETON_EQ_BOUND_INDUCED
        end
        return source == DOUBLETON_EQ_BOUND_INDUCED ?
               (0.0, z_keep_hat / alpha) :
...
@inline function _doubleton_eq_deleted_row_dual(
    y_org,
    AT_rowPtr,
    AT_colVal,
    AT_nzVal,
    ...
    sum_other = 0.0
    col_start = Int(@inbounds AT_rowPtr[elim_col])
    col_stop = Int(@inbounds AT_rowPtr[elim_col + 1]) - 1
    for p in col_start:col_stop
        r = Int(@inbounds AT_colVal[p])
        r == row && continue
        sum_other += @inbounds AT_nzVal[p] * y_org[r]
    end
    return (@inbounds(c[elim_col]) - sum_other - z_elim) / elim_coeff
end

@inline function _doubleton_eq_deleted_row_dual_from_tape(
    y_org,
    tape_idx,
    tape_val,
    idx0::Int,
    ...
    support_count = Int(@inbounds tape_idx[idx0 + 3])
    sum_other = 0.0
    for t in 1:support_count
        row = Int(@inbounds tape_idx[idx0 + 3 + t])
        coeff = @inbounds tape_val[val0 + 11 + t]
        sum_other += coeff * @inbounds y_org[row]
    end
    elim_obj = @inbounds tape_val[val0 + 9]
    ...
    for k in Int(record_count):-1:1
        ...
        elseif reduction_type == Int32(DOUBLETON_EQ)
            if tid == 1
                elim_col = Int(@inbounds tape_idx[idx0])
                keep_col = Int(@inbounds tape_idx[idx0 + 1])
                elim_coeff = @inbounds tape_val[val0]
                keep_coeff = @inbounds tape_val[val0 + 1]
                rhs = @inbounds tape_val[val0 + 2]
                alpha = -keep_coeff / elim_coeff
                beta = rhs / elim_coeff
                @inbounds x_org[elim_col] = beta + alpha * x_org[keep_col]
            end
        elseif reduction_type == Int32(PARALLEL_COL)
            if tid == 1
                from_idx = Int(@inbounds tape_idx[idx0])
        ...
        elseif reduction_type == Int32(FME_COL)
            if tid == 1
                K = Int(@inbounds tape_idx[idx0])
                elim_col = Int(@inbounds tape_idx[idx0 + 1])
                L = Int(round(@inbounds tape_val[val0]))
                ...
                has_hi = false
                for _ in 1:L
                    orient = @inbounds tape_val[vpos]
                    len = Int(round(@inbounds tape_val[vpos + 1]))
                    cand = @inbounds tape_val[vpos + 2]
                    for t in 1:len
                        row_col = Int(@inbounds tape_idx[col_cursor])
                        coeff = @inbounds tape_val[vpos + 2 + t]
                        cand += coeff * @inbounds(x_org[row_col])
                        col_cursor += 1
                    end
                    if orient > 0.0
                        lo = max(lo, cand)
                ...
                @inbounds x_org[elim_col] = has_lo ? lo : (has_hi ? hi : 0.0)
            end
        end
    ...
    if blockIdx().x == 1 && threadIdx().x == 1
        for k in Int(record_count):-1:1
            ...
            elseif reduction_type == Int32(PARALLEL_COL)
                ...
                @inbounds z_org[from_idx] = ratio * z_org[to_idx]
            elseif reduction_type == Int32(DOUBLETON_EQ)
                row = Int(@inbounds tape_idx[idx0 + 2])
                elim_col = Int(@inbounds tape_idx[idx0])
                pivot_coeff = @inbounds tape_val[val0]
                elim_obj = @inbounds tape_val[val0 + 9]
                @inbounds y_org[row] = elim_obj / pivot_coeff
            ...
            elseif reduction_type == Int32(FME_COL)
                ...
                @inbounds z_org[elim_col] = 0.0
                for t in 1:K
                    row = Int(@inbounds tape_idx[idx0 + 1 + t])
                    @inbounds y_org[row] = 0.0
                end
...
function _kernel_replay_dual_tape_all_with_bound_changes!(
    ...
    record_count,
    tol,
    parallel_row_doc_recovery_flag,
)
    if blockIdx().x == 1 && threadIdx().x == 1
        for k in Int(record_count):-1:1
            reduction_type = @inbounds reduction_types[k]
            ...
            elseif reduction_type == Int32(DOUBLETON_EQ)
                ...
                keep_col = Int(@inbounds tape_idx[idx0 + 1])
                row = Int(@inbounds tape_idx[idx0 + 2])
                has_support_payload = idx1 >= idx0 + 3 && val1 >= val0 + 11
                elim_coeff = @inbounds tape_val[val0]
                keep_coeff = @inbounds tape_val[val0 + 1]
                old_keep_l = @inbounds tape_val[val0 + 5]
                old_keep_u = @inbounds tape_val[val0 + 6]
                new_keep_l = @inbounds tape_val[val0 + 7]
                new_keep_u = @inbounds tape_val[val0 + 8]
                lower_source = val1 >= val0 + 10 ? @inbounds(tape_val[val0 + 10]) : DOUBLETON_EQ_BOUND_UNKNOWN
                upper_source = val1 >= val0 + 11 ? @inbounds(tape_val[val0 + 11]) : DOUBLETON_EQ_BOUND_UNKNOWN
                alpha = -keep_coeff / elim_coeff
                z_keep_hat = @inbounds z_org[keep_col]
                z_keep, z_elim = _doubleton_eq_recover_z_split(
                    @inbounds(x_org[keep_col]),
                    z_keep_hat,
                    alpha,
                    old_keep_l,
                    old_keep_u,
                ...
                y_row = has_support_payload ?
                    ...
                    _doubleton_eq_deleted_row_dual(
                        y_org,
                        AT_rowPtr,
                        AT_colVal,
                        AT_nzVal,
                ...
                @inbounds y_org[row] = y_row
                @inbounds z_org[keep_col] = z_keep
                @inbounds z_org[elim_col] = z_elim
                @inbounds z_retrieved[keep_col] = UInt8(1)
                @inbounds z_retrieved[elim_col] = UInt8(1)
            elseif reduction_type == Int32(FIXED_COL)
                idx1 = Int(@inbounds idx_starts[k + 1]) - 1
                col = Int(@inbounds tape_idx[idx0])
            ...
            elseif reduction_type == Int32(PARALLEL_ROW)
                if idx1 >= idx0 + 1 && val1 >= val0 + 4
                    ...
                    if parallel_row_doc_recovery_flag != Int32(0)
                        ybar_p = @inbounds y_org[kept_row]
                        yp_doc, yd_doc, handled = _parallel_row_doc_dual_split(
                            ybar_p,
                            ratio,
                            kept_old_AL,
                            kept_old_AU,
                        ...
                        if handled != UInt8(0)
                            @inbounds y_org[kept_row] = yp_doc
                            @inbounds y_org[deleted_row] = yd_doc
                        end
                    end
                end
...
function postsolve_replay_dual_tape_gpu!(
    ...
    if original_model_gpu === nothing
        ...
        @cuda threads=1 blocks=1 _kernel_replay_dual_tape_all_with_bound_changes!(
            ...
            Int32(record_count),
            tol,
            Int32(parallel_row_doc_recovery ? 1 : 0),
        )
    end
    return (y_org, z_org)
end
...
function postsolve_gpu(
    ...
    postsolve_restore_structural_primal_gpu!(x_org, rec, original_model_gpu)
    ...
    postsolve_replay_dual_tape_gpu!(
        ...
        tol=postsolve_tol,
        parallel_row_doc_recovery=_params.parallel_row_doc_recovery,
    )
...
@inline function _parallel_row_doc_dual_split(
    ybar_p::Float64,
    ratio::Float64,
    kept_old_AL::Float64,
    kept_old_AU::Float64,
    deleted_scaled_AL::Float64,
    deleted_scaled_AU::Float64,
    tol::Float64,
)
    if abs(ybar_p) <= tol
    ...
    bar_l = max(kept_old_AL, deleted_scaled_AL)
    bar_u = min(kept_old_AU, deleted_scaled_AU)
    if !(bar_l <= bar_u + tol)
        return (ybar_p, 0.0, UInt8(0))
    end

    lower_tightened = bar_l > kept_old_AL + tol
    upper_tightened = bar_u < kept_old_AU - tol

    # Document condition: a deleted parallel row should not tighten both sides simultaneously.
    if lower_tightened && upper_tightened
        return (ybar_p, 0.0, UInt8(0))
    end

    if !lower_tightened && !upper_tightened
        return (ybar_p, 0.0, UInt8(1))
    end

    if ybar_p > 0.0
        if lower_tightened
            return (0.0, ratio * ybar_p, UInt8(1))
        end
        return (ybar_p, 0.0, UInt8(1))
    end
...
function _refine_doubleton_eq_dual_from_original!(
    x_org::Vector{Float64},
    y_org::Vector{Float64},
    z_org::Vector{Float64},
    ATy::Vector{Float64},
    ...
    new_keep_l::Float64,
    new_keep_u::Float64;
    lower_source::Float64=DOUBLETON_EQ_BOUND_UNKNOWN,
    upper_source::Float64=DOUBLETON_EQ_BOUND_UNKNOWN,
    tol::Float64=1e-7,
)
    1 <= row <= length(y_org) || return (y_org, z_org)
    1 <= elim_col <= length(z_org) || return (y_org, z_org)
    1 <= keep_col <= length(z_org) || return (y_org, z_org)
    abs(elim_coeff) > tol || return (y_org, z_org)

    alpha = -keep_coeff / elim_coeff
    z_keep_hat = z_org[keep_col] + alpha * z_org[elim_col]
    z_keep, z_elim = _doubleton_eq_recover_z_split(
        x_org[keep_col],
        z_keep_hat,
        alpha,
    ...
    sum_other = 0.0
    rowvals_A = rowvals(original_model.A)
    nzvals_A = nonzeros(original_model.A)
    for p in nzrange(original_model.A, elim_col)
        r = Int(rowvals_A[p])
        r == row && continue
        sum_other += nzvals_A[p] * y_org[r]
    end

    y_old = y_org[row]
    y_new = (original_model.c[elim_col] - sum_other - z_elim) / elim_coeff
    y_org[row] = y_new
    _update_ATy_from_row_delta!(ATy, original_model, row, y_new - y_old; tol=tol)
...
function postsolve_refine_duals_from_original!(
    ...
    for k in record_count:-1:1
        ...
        elseif reduction_type == DOUBLETON_EQ
            length(idx) >= 3 || continue
            length(vals) >= 9 || continue

            _refine_doubleton_eq_dual_from_original!(
                x_org,
                y_org,
                z_org,
                ATy,
                ...
                vals[8],
                vals[9];
                lower_source=length(vals) >= 11 ? vals[11] : DOUBLETON_EQ_BOUND_UNKNOWN,
                upper_source=length(vals) >= 12 ? vals[12] : DOUBLETON_EQ_BOUND_UNKNOWN,
                tol=tol,
            )
