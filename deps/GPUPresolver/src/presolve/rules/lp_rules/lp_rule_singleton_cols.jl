...
- use the implied-free two-sided inequality elimination branch
...
const _SINGLETON_COL_RULE_EQ = UInt8(1)
const _SINGLETON_COL_RULE_DUAL_INFER = UInt8(2)
...
function _kernel_singleton_col_row_owner!(
    ...
    if j <= n
        ...
        @inbounds cj = c_cur[j]
        if rule_mode != _SINGLETON_COL_RULE_EQ &&
        ...
        if is_eq_row
            rule_mode == _SINGLETON_COL_RULE_DUAL_INFER && return
            ...
            rule_mode == _SINGLETON_COL_RULE_EQ && return

            if rule_mode == _SINGLETON_COL_RULE_DUAL_INFER
                ...
                (impl_free_from_above && impl_free_from_below) || return
                chosen = _singleton_col_active_side(c_cur[j], a, lhs, rhs, zero_tol)
                isfinite(chosen) || return
            else
                impl_free_from_above = _singleton_col_implied_free_from_above(
                    a,
                    lhs,
...
function _kernel_process_singleton_cols!(
    ...
    if j <= n
        ...
        if rule_mode == _SINGLETON_COL_RULE_DUAL_INFER
            ...
            isfinite(action_side) || return

            if !isfinite(lhs) || abs(lhs - action_side) > tol
                @inbounds row_lhs_change[row] = UInt8(1)
            end
            if !isfinite(rhs) || abs(rhs - action_side) > tol
                @inbounds row_rhs_change[row] = UInt8(1)
            end
            @inbounds AL_new[row] = action_side
            @inbounds AU_new[row] = action_side
        else
            impl_free_from_above = _singleton_col_implied_free_from_above(
                a,
                lhs,
...
function _apply_rule_singleton_cols_mode!(
    plan::PresolvePlan_gpu,
    lp::LP_info_gpu,
    stats::PresolveStats_gpu,
    ...
    singleton_mask = UInt8.((plan.keep_col_mask .!= UInt8(0)) .& (stats.singleton_col_mask .!= UInt8(0)))
    _, singleton_cols, singleton_count = build_maps_from_mask(singleton_mask)
    Int(singleton_count) == 0 && return nothing

    support_rows = gather_by_red2org(stats.singleton_col_row, singleton_cols)
    support_al = gather_by_red2org(plan.new_AL, support_rows)
    support_au = gather_by_red2org(plan.new_AU, support_rows)
    support_eq = UInt8.(
        isfinite.(support_al) .&
        isfinite.(support_au) .&
        (abs.(support_au .- support_al) .<= pparams.bound_tol)
    )

    if rule_mode == _SINGLETON_COL_RULE_EQ
        Int(sum(Int32.(support_eq .!= UInt8(0)))) == 0 && return nothing
    else
        Int(singleton_count) == Int(sum(Int32.(support_eq .!= UInt8(0)))) && return nothing
    end
    ...
    AL_old = copy(plan.new_AL)
    AU_old = copy(plan.new_AU)
    ...
    row_tighten_mask = UInt8.((row_lhs_change .!= UInt8(0)) .| (row_rhs_change .!= UInt8(0)))
    _, tightened_rows, tightened_count = build_maps_from_mask(row_tighten_mask)
    ...
    pair_rows = Int(eliminated_count) > 0 ? gather_by_red2org(pair_row, eliminated_cols) : CuVector{Int32}(undef, 0)
    chosen_side_sel = Int(eliminated_count) > 0 ? gather_by_red2org(chosen_side, eliminated_cols) : CuVector{Float64}(undef, 0)
    old_l = Int(eliminated_count) > 0 ? gather_by_red2org(plan.new_l, eliminated_cols) : CuVector{Float64}(undef, 0)
    old_u = Int(eliminated_count) > 0 ? gather_by_red2org(plan.new_u, eliminated_cols) : CuVector{Float64}(undef, 0)
    elim_obj = Int(eliminated_count) > 0 ? gather_by_red2org(plan.new_c, eliminated_cols) : CuVector{Float64}(undef, 0)
    if pparams.record_postsolve_tape
        if rule_mode == _SINGLETON_COL_RULE_DUAL_INFER
            if Int(tightened_count) > 0
            ...
            if Int(lhs_changed_count) > 0
                for t in 1:Int(lhs_changed_count)
                    ...
                    old_al = CUDA.@allowscalar Float64(AL_old[row])
                    old_au = CUDA.@allowscalar Float64(AU_old[row])
    ...
    row_changed = Int(eliminated_count) > 0 || row_tightened
    col_changed = Int(eliminated_count) > 0
    row_changed && (plan.has_row_action = true)
    col_changed && (plan.has_col_action = true)
    (row_changed || col_changed) && (plan.has_change = true)
    return nothing
end

"""
Rule: equality-row singleton-column elimination by substitution.
"""
function apply_rule_singleton_cols_eq!(
    plan::PresolvePlan_gpu,
    lp::LP_info_gpu,
    stats::PresolveStats_gpu,
    pparams::PresolveParams,
)
    return _apply_rule_singleton_cols_mode!(
        plan,
        lp,
        stats,
        pparams,
...
"""
Rule: strict dual inference for inequality-row singleton columns.
"""
function apply_rule_singleton_cols_dual_infer!(
    plan::PresolvePlan_gpu,
    lp::LP_info_gpu,
    stats::PresolveStats_gpu,
    pparams::PresolveParams,
    ...
    return _apply_rule_singleton_cols_mode!(
        ...
        stats,
        pparams,
        _SINGLETON_COL_RULE_DUAL_INFER,
    )
end
