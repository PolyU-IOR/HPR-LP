...
function _kernel_parallel_row_groups!(
    ...
    if s <= m
        ...
        for a in s:e
            ...
            if rep_seed == Int32(0) || (row_is_eq && !rep_seed_is_eq)
                rep_seed = row
                rep_seed_is_eq = row_is_eq
            end
        end

        rep_seed == Int32(0) && return

        # Phase 1: choose a stronger representative inside the hash bucket.
        rep = rep_seed
        @inbounds rep_seed_l = AL[rep_seed]
        @inbounds rep_seed_u = AU[rep_seed]
        rep_scaled_l = rep_seed_l
        rep_scaled_u = rep_seed_u

        for a in s:e
            @inbounds row = sorted_rows[a]
            if row == rep_seed || keep_row[row] == UInt8(0)
                continue
            end

            is_parallel, ratio_seed = _parallel_row_ratio(
                rep_seed,
                row,
                row_ptr,
                col_val,
            ...
            row_scaled_l, row_scaled_u = _scaled_interval_for_parallel_row(
                @inbounds(AL[row]),
                @inbounds(AU[row]),
                ratio_seed,
            )
        ...
        @inbounds rep_l = AL[rep]
        @inbounds rep_u = AU[rep]
