...
const DOUBLETON_EQ_BOUND_UNKNOWN = 0.0
const DOUBLETON_EQ_BOUND_INHERITED = 1.0
const DOUBLETON_EQ_BOUND_INDUCED = 2.0
const DOUBLETON_EQ_BOUND_BOTH = 3.0
...
"""
Record a Fourier-Motzkin eliminated column for postsolve.

`elim_col` is the eliminated column; `involved_rows` are the (to-be-deleted)
constraint rows it appeared in. `sides` describes the inequality sides that
bound the column, each given as `(orient, rhs, cols, coeffs)` where
`x_elim {>= if orient>0, <= if orient<0} rhs + sum(coeffs .* x[cols])`.

Primal recovery picks `x_elim = max(lower-side candidates)` if any lower side
exists, else `min(upper-side candidates)`, else `0`; this is always feasible
because every (lower, upper) pair was projected out as a kept constraint.

Payload layout (indices): `[K, elim_col, involved_rows(K)..., S, side_cols(S)...]`
where `K = length(involved_rows)` and `S` is the total number of side-column
entries. Values: `[L, (orient, len, rhs, coeffs(len)...) for each of L sides]`,
with `side_cols` concatenated in the same side order.
"""
function append_fme_col_record!(
    tape::PostsolveTape,
    elim_col::Integer,
    involved_rows::AbstractVector{<:Integer},
    sides::AbstractVector{<:NamedTuple};
    dual_mode::PostsolveDualMode=POSTSOLVE_DUAL_MINIMAL,
)
    K = length(involved_rows)
    L = length(sides)
    side_cols = Int32[]
    for s in sides
        append!(side_cols, Int32.(s.cols))
    end
    S = length(side_cols)

    idx = Int32[Int32(K), Int32(elim_col)]
    append!(idx, Int32.(involved_rows))
    push!(idx, Int32(S))
    append!(idx, side_cols)

    vals = Float64[Float64(L)]
    for s in sides
        push!(vals, Float64(s.orient))
        push!(vals, Float64(length(s.cols)))
        push!(vals, Float64(s.rhs))
        append!(vals, Float64.(s.coeffs))
    end
...
function append_eq_to_ineq_records_gpu!(
    tape::PostsolveTape_gpu,
    rows::CuVector{Int32};
    dual_mode::PostsolveDualMode=POSTSOLVE_DUAL_MINIMAL,
)
    ...
    types = CUDA.fill(Int32(EQ_TO_INEQ), k)
    index_starts = _build_constant_record_starts_gpu(k, Int32(1))
    value_starts = _build_constant_record_starts_gpu(k, Int32(1))
    dual_modes = CUDA.fill(UInt8(dual_mode), k)
    indices = copy(rows)
    vals = CUDA.zeros(Float64, k)
...
function _kernel_pack_activity_check_row_records!(
    indices,
    vals,
    rows,
    old_AL,
    ...
    t = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    if t <= k
        @inbounds indices[t] = rows[t]
        if value_width == Int32(2)
            val0 = 2 * (t - 1) + 1
            ...
            @inbounds vals[val0] = old_AL[t]
            @inbounds vals[val0 + 1] = old_AU[t]
            @inbounds vals[val0 + 2] = new_AL[t]
            @inbounds vals[val0 + 3] = new_AU[t]
        end
    end
    return
end

function append_activity_check_row_records_gpu!(
    tape::PostsolveTape_gpu,
    reduction_type::PostsolveReductionType,
    rows::CuVector{Int32},
    ...
    dual_mode::PostsolveDualMode=POSTSOLVE_DUAL_MINIMAL,
    cpu_tape::Union{Nothing,PostsolveTape}=nothing,
)
    k = length(rows)
    k == 0 && return tape
    if !(reduction_type in (DELETED_ROW, LHS_CHANGE, RHS_CHANGE))
        error("Unsupported activity-check row tape type $(reduction_type).")
    end

    value_width = reduction_type == DELETED_ROW ? Int32(2) : Int32(4)
    types = CUDA.fill(Int32(reduction_type), k)
    index_starts = _build_constant_record_starts_gpu(k, Int32(1))
    value_starts = _build_constant_record_starts_gpu(k, value_width)
    dual_modes = CUDA.fill(UInt8(dual_mode), k)
    indices = CuVector{Int32}(undef, k)
    vals = CuVector{Float64}(undef, Int(value_width) * k)

    blocks = cld(k, GPU_PRESOLVE_THREADS)
    @cuda threads=GPU_PRESOLVE_THREADS blocks=blocks _kernel_pack_activity_check_row_records!(
        indices,
        vals,
        rows,
        old_AL,
    ...
    record = PostsolveTape_gpu(types, index_starts, value_starts, dual_modes, indices, vals)
    append_postsolve_tape!(tape, record)
    if cpu_tape !== nothing
        append_postsolve_tape!(cpu_tape, PostsolveTape(record))
    end
    return tape
end
...
function append_doubleton_eq_record!(
    tape::PostsolveTape,
    elim_col::Integer,
    keep_col::Integer,
    row::Integer,
    ...
    new_keep_u::Real,
    elim_obj::Real;
    support_rows::AbstractVector{<:Integer}=Int32[],
    support_coeffs::AbstractVector{<:Real}=Float64[],
    lower_source::Real=DOUBLETON_EQ_BOUND_UNKNOWN,
    upper_source::Real=DOUBLETON_EQ_BOUND_UNKNOWN,
    dual_mode::PostsolveDualMode=POSTSOLVE_DUAL_MINIMAL,
)
    length(support_rows) == length(support_coeffs) || error("DOUBLETON_EQ payload mismatch.")
    idx = Int32[elim_col, keep_col, row, length(support_rows)]
    append!(idx, Int32.(support_rows))
    vals = Float64[
        elim_coeff,
        keep_coeff,
    ...
    append!(vals, Float64.(support_coeffs))
    return append_postsolve_record!(
        tape,
        DOUBLETON_EQ;
        indices=idx,
...
function globalize_postsolve_tape(
    ...
    for k in 1:postsolve_record_count(tape)
        ...
        elseif reduction_type == DOUBLETON_EQ
            ...
            idx[3] = row_map[idx[3]]
            if length(idx) >= 4
                support_count = Int(idx[4])
                for t in 1:support_count
                    idx[4 + t] = row_map[idx[4 + t]]
                end
            end
        elseif reduction_type == FME_COL
            K = Int(idx[1])
            idx[2] = col_map[idx[2]]
            for t in 1:K
                idx[2 + t] = row_map[idx[2 + t]]
            end
            S = Int(idx[3 + K])
            for t in 1:S
                idx[3 + K + t] = col_map[idx[3 + K + t]]
            end
        end
...
function _kernel_globalize_postsolve_tape_indices!(
    ...
    if k <= record_count
        ...
        elseif reduction_type == Int32(DOUBLETON_EQ)
            @inbounds mapped_idx[idx0] = col_map[mapped_idx[idx0]]
            @inbounds mapped_idx[idx0 + 1] = col_map[mapped_idx[idx0 + 1]]
            @inbounds mapped_idx[idx0 + 2] = row_map[mapped_idx[idx0 + 2]]
            if idx1 >= idx0 + 3
                support_count = Int(@inbounds mapped_idx[idx0 + 3])
                for t in 1:support_count
                    pos = idx0 + 3 + t
                    pos <= idx1 || break
                    @inbounds mapped_idx[pos] = row_map[mapped_idx[pos]]
        ...
        elseif reduction_type == Int32(FME_COL)
            K = Int(@inbounds mapped_idx[idx0])
            @inbounds mapped_idx[idx0 + 1] = col_map[mapped_idx[idx0 + 1]]
            for t in 1:K
                @inbounds mapped_idx[idx0 + 1 + t] = row_map[mapped_idx[idx0 + 1 + t]]
            end
            S = Int(@inbounds mapped_idx[idx0 + 2 + K])
            for t in 1:S
                @inbounds mapped_idx[idx0 + 2 + K + t] = col_map[mapped_idx[idx0 + 2 + K + t]]
            end
        end
    end
    return
